...
class StrategyTests: XCTestCase {
  ...
  func testRandom() {
    let game = Begriffix(startLetters: "xxxx", starter: randomPlayer.move, opponent: randomPlayer.move)
    let move = randomBegriffixStrategy(hits: hits, game: game)
    XCTAssertNotNil(move)
  }
  func testShort() {
    let game = Begriffix(startLetters: "xxxx", starter: shortPlayer.move, opponent: shortPlayer.move)
    let move = shortBegriffixStrategy(hits: hits, game: game)
    XCTAssertEqual(move?.place.count, 4)
  }
  func testLong() {
    let game = Begriffix(startLetters: "xxxx", starter: longPlayer.move, opponent: longPlayer.move)
    let move = longBegriffixStrategy(hits: hits, game: game)
    XCTAssertEqual(move?.place.count, 8)
  }
  func testAvailability() {
    let game = Begriffix(startLetters: "xxxx", starter: availabilityPlayer.move, opponent: availabilityPlayer.move)
